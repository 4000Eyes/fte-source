...
class SearchDB:
    ...
    def search_gemift_products(self, inputs, output_list):
        ...
        try:
            ...
            if "occasion" in inputs:
                occasions_hash = {}
                occasions_hash["text"] = {}
                occasions_hash["text"]["query"] = [inputs["occasions"]]
    ...
    def search_by_occasion(self, inputs, output_list):
        ...
        try:
            ...
            result = user_collection.aggregate([{"$search": {"index": self.get_index(),
                                                             "compound": {"must": [{"text": {
                                                                 "query": [inputs["occasion_names"]], "path": "occasion"}},
                                                                                   {"compound": {"filter": [
                                                                                       {"range": {
    ...
    def search_by_occasion_color(self, inputs, output_list):
        ...
        try:
            ...
            result = None
            test = "prod_index"
            vartex = '{"age_lo": -1, "created_dt": -1, "price": sort_order}'
            user_collection = pymongo.collection.Collection(g.db, self.get_search_collection())
            result = user_collection.aggregate([{"$search": {"index": self.get_index(),
                                                             "compound": {"must": [{"text": {
